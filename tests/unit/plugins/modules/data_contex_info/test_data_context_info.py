...
from ansible_collections.cloudera.cluster.plugins.modules import data_context_info
...
def test_data_context_info(module_args, conn):
    conn.update(
    ...
    with pytest.raises(AnsibleExitJson) as e:
        data_context_info.main()

    LOG.info(str(e.value.data_context_info))


def test_data_context_info_all_hosts(module_args, conn):
    conn.update(
        host=os.getenv("CM_HOST"),
    )
